...
template<> Array<uint8_t,3,RowMajorOrder> nd::image::load_image(const std::string& filename,unsigned int desired_channels)
{
	int x,y,c;
...
template<> Array<float,3,RowMajorOrder> nd::image::load_image(const std::string& filename,unsigned int desired_channels)
{
	int x,y,c;
...
template<> Array<uint16_t,3,RowMajorOrder> nd::image::load_image(const std::string& filename,unsigned int desired_channels)
{
	int x,y,c;
...
template<> void nd::image::save_image(const Array<uint8_t,3,RowMajorOrder>& img,const std::string& filename)
{
	char ext=get_ext(filename);
	if(ext=='h' && ext=='H')
	{
		Array<float,3,RowMajorOrder> cimg;
		cimg=img;
	...
	switch(ext)
	{
		case 'p':
		case 'P':
			result=stbi_write_png(filename.c_str(),img.shape(1),img.shape(0),img.shape(2),img.data(),img.shape(2)*img.shape(1)*sizeof(uint8_t));
			break;
		case 't':
		case 'T':
			result=stbi_write_tga(filename.c_str(),img.shape(1),img.shape(0),img.shape(2),img.data());
			break;
		case 'j':
		case 'J':
			result=stbi_write_jpg(filename.c_str(),img.shape(1),img.shape(0),img.shape(2),img.data(),0);
			break;
		case 'b':
		case 'B':
			result=stbi_write_bmp(filename.c_str(),img.shape(1),img.shape(0),img.shape(2),img.data());
			break;
		default:
...
template<> void nd::image::save_image(const Array<float,3,RowMajorOrder>& img,const std::string& filename)
{
	char ext=get_ext(filename);
	if(ext!='h' && ext!='H')
	{
		Array<uint8_t,3,RowMajorOrder> cimg;
		cimg=img;
		save_image(cimg,filename);
		return;
	}
...
/*template<> void nd::image::save_image(const Array<uint16_t,3,RowMajorOrder>& img,const std::string& filename)
{
